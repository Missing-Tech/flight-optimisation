...
if __name__ == "__main__":
    ...
    ant_paths, aco_path, objectives_dataframe, best_indexes = aco.run_aco_colony(
    ...
    real_flight_path = fp.calculate_flight_characteristics(real_flight_path, metGrid)
    ...
    aco_objective = aco.calculate_objective_dataframe(aco_path, contrail_grid)
    fp_objective = aco.calculate_objective_dataframe(real_flight_path, contrail_grid)
    print(f"ACO Objective: {aco_objective}")
    print(f"FP Objective: {fp_objective}")
    ...
    # display.display_routing_grid(rg.get_routing_grid(), ax=ax_side_2)
    # display.display_flight_path(real_flight_path, ax_map)
    # display.display_geodesic_path(gp.get_geodesic_path(), ax=ax_side_3)
